...
<xsl:param    name="titleReport" select="'API接口自动化测试报告'" />
	...
	<html>
		<head>
			...
			<style type="text/css">
				...
				body,td,tr,th,h1,h2,h3,xsl:template {
					font-family:Arial
				}
				table.details tr th{
				    color: #ffffff;
				...
				h1 {
				    background:#00aeef;
					color:#FFFFFF;
					margin: 0px 0px 0px; font: 350% verdana,arial,helvetica;
					font-family:Arial
				}
				h2 {
					margin-top: 1em; margin-bottom: 0em; font: bold 150% verdana,arial,helvetica;
...
<xsl:template name="detail">
	...
	<xsl:if test="$allFailureCount > 0">
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<xsl:if test="$failureCount > 0">
				<!--h3>接口：<xsl:value-of select="@lb" /><a><xsl:attribute name="name"><xsl:value-of select="@lb" /></xsl:attribute></a>-->
				...
				<xsl:for-each select="/testResults/*[@lb = current()/@lb][attribute::s='false']">
					<tr>
					    <td><xsl:value-of select="@lb" /><a><xsl:attribute name="name"><xsl:value-of select="@lb" /></xsl:attribute></a></td>
